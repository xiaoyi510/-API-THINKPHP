...
<div class="container-full" style="margin-left: 250px;position: absolute;top: 0px;width: calc(100% - 250px);">
    ...
    <a href="<?php echo U('add');?>" class="btn btn-info">添加管理员</a>
    <table class="table table-condensed table-hover table-responsive table-bordered">
        ...
        <?php if(is_array($userList)): $i = 0; $__LIST__ = $userList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><?php echo $user['is_super']==0?'普通用户':($user['is_super']==1?'管理员':'超级管理员');?></td>
                <td>
                    <a href="<?php echo U('edit',['id'=>$user['id']]);?>">编辑</a>
                    <a href="<?php echo U('remove',['id'=>$user['id']]);?>">删除</a>
                </td>
